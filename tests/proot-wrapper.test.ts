...
describe('ProotDistroManager', () => {
  describe('isProotDistroInstalled', () => {
    test('should return true when proot-distro command exists', () => {
      const runner = createMockRunner([
        { pattern: 'command -v proot-distro', result: { stdout: '/usr/bin/proot-distro\n', stderr: '', exitCode: 0 } },
      ]);
      expect(new ProotDistroManager(runner).isProotDistroInstalled()).toBe(true);
    });

    test('should return false when proot-distro not found', () => {
      const runner = createMockRunner([
        { pattern: 'command -v proot-distro', result: { stdout: '', stderr: 'not found', exitCode: 1 } },
      ]);
      expect(new ProotDistroManager(runner).isProotDistroInstalled()).toBe(false);
    });
  });
  ...
  describe('isDistroInstalled', () => {
    test('should return true when distro is in installed list', () => {
      const runner = createMockRunner([
        { pattern: 'command -v proot-distro', result: { stdout: '/usr/bin/proot-distro\n', stderr: '', exitCode: 0 } },
        { pattern: 'list --installed', result: { stdout: 'ubuntu\n', stderr: '', exitCode: 0 } },
      ]);
      expect(new ProotDistroManager(runner).isDistroInstalled('ubuntu')).toBe(true);
    });

    test('should return false when distro is not installed', () => {
      const runner = createMockRunner([
        { pattern: 'command -v proot-distro', result: { stdout: '/usr/bin/proot-distro\n', stderr: '', exitCode: 0 } },
        { pattern: 'list --installed', result: { stdout: '', stderr: '', exitCode: 1 } },
      ]);
      expect(new ProotDistroManager(runner).isDistroInstalled('debian')).toBe(false);
    });
  });
  ...
  describe('getInstalledDistros', () => {
    test('should return array of distro names', () => {
      ...
      expect(new ProotDistroManager(runner).getInstalledDistros()).toEqual(['ubuntu', 'debian']);
    });

    test('should return empty array on failure', () => {
      const runner = createMockRunner([
        { pattern: 'list --installed', result: { stdout: '', stderr: 'error', exitCode: 1 } },
      ]);
  ...
  describe('installDistro', () => {
    test('should return error when proot-distro not installed', () => {
      const runner = createMockRunner([
        { pattern: 'command -v proot-distro', result: { stdout: '', stderr: '', exitCode: 1 } },
      ]);
  ...
  describe('preflightCheck', () => {
    ...
    test('should report ready when all components present', () => {
      const runner = createMockRunner([
        { pattern: 'command -v proot-distro', result: { stdout: '/usr/bin/proot-distro\n', stderr: '', exitCode: 0 } },
        { pattern: /list --installed.*grep/, result: { stdout: 'ubuntu\n', stderr: '', exitCode: 0 } },
        { pattern: 'list --installed', result: { stdout: 'ubuntu\n', stderr: '', exitCode: 0 } },
        { pattern: 'command -v freebuff', result: { stdout: '/root/.bun/bin/freebuff\n', stderr: '', exitCode: 0 } },
      ]);
      const check = new ProotDistroManager(runner).preflightCheck('ubuntu');
  ...
  describe('runFreeBuff', () => {
    test('should convert Termux CWD to proot CWD in result', () => {
      const runner = createMockRunner([
        { pattern: 'command -v proot-distro', result: { stdout: '/usr/bin/proot-distro\n', stderr: '', exitCode: 0 } },
        { pattern: /list --installed.*grep/, result: { stdout: 'ubuntu\n', stderr: '', exitCode: 0 } },
        { pattern: 'proot-distro login', result: { stdout: 'FreeBuff output', stderr: '', exitCode: 0 } },
      ]);
      const termuxHome = '/data/data/com.termux/files/home';
      const result = new ProotDistroManager(runner).runFreeBuff(
        'ubuntu', `${termuxHome}/my-project`, [], { termuxHome, prootHome: '/root' }
      );
